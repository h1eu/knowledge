...
| Role | Purpose | Light (Crisp Porcelain) | Dark (Linear Titanium, default) |
|---|---|---|---|
| `paper` | Page background, default node fill | `#F8FAFC` | `#060913` |
| `paper-2` | Diagram container bg, secondary fill | `#FFFFFF` | `#0B132B` |
| `ink` | Primary text, primary stroke | `#0F172A` | `#F8FAFC` |
| `muted` | Secondary text, default arrow stroke | `#64748B` | `#94A3B8` |
| `soft` | Sublabels, boundary labels | `#94A3B8` | `#64748B` |
| `rule` | Hairline borders | `rgba(15,23,42,0.12)` | `rgba(248,250,252,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#94A3B8` | `rgba(148,163,184,0.40)` |
| `accent` | Focal / 1–2 max per diagram | `#0284C7` (deep sky) | `#38BDF8` (sky) |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(2,132,199,0.08)` | `rgba(56,189,248,0.12)` |
| `link` | HTTP/API calls, external arrows | `#7C3AED` (violet) | `#A78BFA` (violet) |

> **Brand palette source:** onboarded 2026-09-05 from the Knowledge OS website (`website/style.css`, folder method) — site is dark-first so the dark column is the default skin. Tokens map to the site's CSS variables: `paper` ← `--bg-base`, `paper-2` ← `--bg-surface`, `ink` ← `--text-primary`, `muted` ← `--text-secondary`, `soft` ← `--text-muted`, `accent` ← `--accent-sky`, `link` ← `--accent-violet`. Contrast: light `ink`/`muted` on `paper` ≈ 15:1 / 4.8:1; dark ≈ 18:1 / 7:1 (WCAG AA ✓). `paper` is `#F8FAFC`, not pure white, per the warm-neutral rule.
...
|---|---|---|---|---|
| `title` | Plus Jakarta Sans | 1.75rem | 800 | Page H1 (site heading face, exact via Google Fonts) |
| `node-name` | Geist (sans) | 12px | 600 | Human-readable labels (site body face, exact via Google Fonts) |
| `sublabel` | JetBrains Mono | 9px | 400 | Port, protocol, URL, field type (site code face; technical slots only, never blanket) |
...
| `callout` | Plus Jakarta Sans *italic* | 14px | 600 | Editorial asides only |

> **Onboarding note:** the site self-hosts these families (`website/fonts/`); diagrams reference the identical families/weights via Google Fonts (all three ship Vietnamese subsets, matching lesson content). `Instrument Serif`/`Geist Mono` defaults were replaced because the brand has exact equivalents; the serif+sans+mono contrast is preserved as sans-black + sans + mono.

### Font stack

```html
<link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:ital,wght@0,400;0,600;0,700;0,800;1,600&family=Geist:wght@400;500;600&family=JetBrains+Mono:wght@400;500;600&display=swap" rel="stylesheet">
```
